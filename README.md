...
随著前端开发技术的蓬勃发展，许多需求大量移往前端来处理、甚至将网页做成类似 Gmail、Google Calendar 的交互型的应用程式。这个趋势使得 CSS 与 JavaScript 代码量大量增加，也得加载许多现成的「库」来满足复杂的需求。本文将结合工具及范例、介绍加载 JavaScript/CSS 的数种方法与模式，进而引出载入前端模块的理想方式。
...
![image](http://farm9.staticflickr.com/8297/7987296815_6bc0450b7a_b.jpg)
* 图 1 - 范例网页：两岸三地的开发者

![image](http://farm9.staticflickr.com/8030/7980180886_c650d9f767_z.jpg)
* 图 2 - 两岸三地的开发者网页的页面模块结构图

在两岸三地的开发者的网页（图 1）、其结构（图 2）共有三个页面模块：上面的「条件筛选模块」(\_filter)、左边的「资讯模块」(\_info)、右侧的「列表模块」(\_list)，都各自有自己的 HTML、CSS、与 JavaScript。
...
图 3 中，虽然每位程序员专注于开发自己的模块、但都得共同修改同一个设定档，此设定档记载了每页要载入的模块有哪些。是一个很直觉的作法。
...
即可输出对应的 link、script 标签与模块依赖关系的设定（如图 5）。
